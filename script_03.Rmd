...
```{r}
sep_new <- read_excel("class_enrollment_summary_by_term_9-22-2020.xlsx", 
             skip = 3) %>% 
  clean_names() %>% 
  filter(! is.na(course_title)) %>% 
  select(-grad, -non_degree, -x_reg, -vus, -employee, 
...
sep_new %>%
  group_by(title) %>%
  summarise(u_grad_sum = sum(u_grad), .groups = "drop")
```
...
# # Know exactly why we are investigating this model
# # Is there somethign wrong with these classes that makes students want to drop them?


After we have a model, Temperance suggests we should:

# # Know that our data has limitations
# # May not be reflective of the real reasons why students drop courses
